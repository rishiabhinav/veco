...
#
# Deploys Keswick ESXi ova to a hypervisor, e.g. ESXi.
...
usage() {
  ...
  usage="
USAGE:
  $(basename "$0") [-v esxi|fusion|ws ] -o keswick.ova -m vm_name [-f keswick_firstboot.yaml]

  Deploys Keswick ESXi .ova as a VM to a hypervisor, e.g. ESXi (directly or
  ...
  Common args:
    ...
    -o : Keswick ESXi .ova file path.
    -m : Keswick ESXi VM name to import ova as.
    -r : Keswick ESXi VM root password (optional).
    -f : Keswick firstboot yaml file path (optional).
    -s : KPS server URL.
...
test -n "$KPSURL" || { fatal "missing -s arg\n$(usage)"; }
  ...
  read -r -d '' options <<EOT || :
{
  ...
  "PropertyMapping": [
    {
      "key": "guestinfo.keswick.firstboot",
      "value": "$KFBYAML"
    },
    {
...
EOT
  ...
  $GOVC vm.change \
    -e "guestinfo.keswick.firstboot=$KFBYAML" \
    -e "guestinfo.root.passwd=$KROOTPWD" \
    -e "guestinfo.keswick.url=$KPSURL" \
...
deploy2local() {
  ...
  test -n "$KFBYAML" && echo "guestinfo.keswick.firstboot=$KFBYAML" >> "$VMX"
  test -n "$KROOTPWD" && echo "guestinfo.root.passwd=$KROOTPWD" >> "$VMX"
  test -n "$KPSURL" && echo "guestinfo.keswick.url=$KPSURL" >> "$VMX"
